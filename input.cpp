...
static float	angle;
static float	padstat_x[GAMEPADMAX];
static float	padstat_y[GAMEPADMAX];
...
void UpdatePad(void)
{
	...
	for (i = 0; i < padCount; i++)
	{
		...
		//* y-axis (forward)
		if (dijs.lRz < 3000)					padState[i] |= BUTTON_UP_R;
		//* y-axis (backward)
		if (dijs.lRz > 62535)					padState[i] |= BUTTON_DOWN_R;
		//* x-axis (left)
		if (dijs.lZ < 3000)					padState[i] |= BUTTON_LEFT_R;
		//* x-axis (right)
		if (dijs.lZ > 62535)				padState[i] |= BUTTON_RIGHT_R;

#ifdef _DEBUG
		PrintDebugProc("lZ:%f lRz:%f\n", (float)dijs.lY, (float)dijs.lX);
#endif // DEBUGPrintDebugProc("Player:X:%f Y:%f Z:%f\n", g_Player.pos.x, g_Player.pos.y, g_Player.pos.z);
...
float GetStickdata(int controllerid, int id)
{
	if (id == 0)
	{
		if (padstat_x[controllerid] < 0)
		{
			return padstat_x[controllerid] * -1.0f;
		}

		return padstat_x[controllerid];
	}
	else
	{

		if (padstat_y[controllerid] < 0)
		{
			return padstat_y[controllerid] * -1.0f;
		}


		return padstat_y[controllerid];
	}
}